...
import Link from 'next/link';
import { usePathname } from 'next/navigation';
import { Lock, Users, BarChart3, FileText, Settings, LogOut, ChevronLeft, ChevronRight } from 'lucide-react';
import { useState, useEffect } from 'react';
import { getCurrentUserProfile, supabase } from '../lib/supabaseClient';
import { useRouter } from 'next/navigation';
...
const Sidebar = () => {
  ...
  const getAvatarDisplay = () => {
    ...
    return (
      <div className={`bg-indigo-600 rounded-full flex items-center justify-center text-white font-semibold ${
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            ...
            <button
              onClick={handleLogout}
              disabled={logoutLoading}
              className={`text-gray-400 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${
                isCollapsed 
            ...
            >
              ...
              {!isCollapsed && logoutLoading && (
                <div className="ml-auto animate-spin rounded-full h-3 w-3 border-b-2 border-red-400"></div>
              )}
              {isCollapsed && logoutLoading && (
                <div className="absolute animate-spin rounded-full h-3 w-3 border-b-2 border-red-400"></div>
              )}
            </button>
          </div>
        ) : (
          <div className="space-y-3">
            ...
            <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-3'}`}>
              <div className={`bg-gray-700 rounded-full flex items-center justify-center text-white font-semibold ${
                isCollapsed ? 'w-8 h-8 text-xs' : 'w-10 h-10 text-sm'
              }`}>
                U
              </div>
            ...
            <Link href="/auth/login" className="block">
              <button
                className={`text-gray-400 hover:text-indigo-400 hover:bg-indigo-500/10 rounded-lg transition-colors ${
                  isCollapsed 
                    ? 'w-full flex justify-center p-2' 
                    : 'w-full flex items-center gap-3 px-3 py-2 text-sm'
                }`}
